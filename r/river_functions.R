...
getTopWidth  <- function(wse = NULL, xsct = NULL, colX = NULL, colZ = NULL) {
  # Interpolate banks
...
# Calculates the number of inundated sections for a given x-sect and wse
nInundate    <- function(xsct = NULL, wse = NULL, colX = NULL, colZ = NULL) {
  xsct <- interpBanks(xsct = xsct, wse = wse, colX = colX, colZ = colZ)
...
createWSE    <- function(xsct, wse, colX, colZ) {
  ...
  stn  <- unique(surf$stn); stn <- stn[complete.cases(stn)]
  surf <- surf[which(is.na(surf$stn)), ]
  naxs <- surf[1, ]; naxs$y2 <- NA
  if (nrow(surf) > 2) {
    nbrk <- nrow(surf) / 2 - 1 # Number of section breaks
    for (n in 1 : nbrk) {
      naxs$stn  <- stn; 
      naxs$x2   <- mean(c(surf$x2[2 * n], surf$x2[2 * n + 1]))
...
getXsctArea  <- function(wse = NULL, xsct = NULL, colX = NULL, colZ = NULL) {
...
getWetPerim  <- function(wse = NULL, xsct = NULL, colX = NULL, colZ = NULL) {
  # Interpolate banks and cull ground above wse
  xsct <- interpBanks(xsct = xsct, wse = wse, colX = colX, colZ = colZ)
  xCol <- which(names(xsct) == colX); zCol = which(names(xsct) == colZ)
  xsct <- xsct[which(xsct[zCol] <= wse), ]
...
getHydrRad   <- function(wse = NULL, xsct = NULL, colX = NULL, colZ = NULL) {
...
getManningsN <- function(wse = NULL, xsct = NULL, colX = NULL, colZ = NULL, 
                         q = NULL, S = NULL) {
